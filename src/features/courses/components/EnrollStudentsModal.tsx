...
export const EnrollStudentsModal = ({ courseId, courseName }: EnrollStudentsModalProps) => {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="w-[95vw] sm:max-w-[900px] p-0 overflow-hidden rounded-[2rem] sm:rounded-[3rem] border-none shadow-2xl bg-white flex flex-col focus:outline-none focus:ring-0 max-h-[90vh]">
        ...
        <div className="relative p-6 sm:p-12 pb-6 sm:pb-8 overflow-hidden bg-slate-50/30">
          ...
          <div className="relative space-y-4 sm:y-8">
            ...
            <div className="space-y-2 sm:space-y-3">
              ...
              <DialogDescription className="text-slate-500 font-bold text-sm sm:text-lg max-w-xl leading-snug">
                Synchronize module access using manual entry or <span className="text-primary italic">CSV acceleration</span>.
              </DialogDescription>
            </div>
          </div>
        </div>
        ...
        {/* Dropzone Interface */}
        <div className="px-6 sm:px-12 py-4 sm:py-6 bg-white border-y border-slate-100">
          <div 
            {...getRootProps()} 
            className={cn(
              "group relative border-2 border-dashed rounded-[1.5rem] sm:rounded-[2.5rem] p-6 sm:p-10 transition-all duration-500 cursor-pointer overflow-hidden",
              isDragActive ? "border-primary bg-primary/5 scale-[0.99]" : "border-slate-200 bg-slate-50/50 hover:border-primary/40 hover:bg-slate-50"
            )}
          >
            <input {...getInputProps()} />
            <div className="relative z-10 flex flex-col items-center justify-center gap-2 sm:gap-4 text-center">
              <div className={cn(
                "w-10 h-10 sm:w-16 sm:h-16 rounded-xl sm:rounded-2xl flex items-center justify-center transition-all duration-500 shadow-sm",
                isDragActive ? "bg-primary text-white scale-110 rotate-12" : "bg-white text-slate-400 group-hover:text-primary group-hover:rotate-12"
              )}>
                {isImporting ? <Loader2 className="w-6 h-6 sm:w-8 sm:h-8 animate-spin" /> : <FileText className="w-6 h-6 sm:w-8 sm:h-8" />}
              </div>
              <div className="space-y-0.5 sm:space-y-1">
                <p className="text-[10px] sm:text-sm font-black uppercase tracking-widest text-slate-800">
                  {isDragActive ? "Infiltrating Data..." : "Drag & Drop Module Roster"}
                </p>
                <p className="text-[8px] sm:text-[11px] font-bold text-slate-400 uppercase tracking-tighter">
