...
import { 
  ...
  TrendingUp, Globe, Shield, FileText, Loader2
} from "lucide-react";
    ...
    <div className="flex flex-col gap-6">
      <div className="flex justify-between items-start">
        ...
        <div className="flex flex-col items-end gap-1">
          ...
          <span className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground/40 italic">Edit Mode</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="p-3 rounded-2xl bg-white/10 dark:bg-black/20 border border-white/10 flex flex-col gap-1 text-right">
          <span className="text-[9px] font-black uppercase tracking-widest text-muted-foreground/60">State</span>
          <span className="text-lg font-black text-amber-500 italic">SYNC</span>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 pt-2">
        <div className="w-full h-1 bg-white/10 dark:bg-white/5 rounded-full overflow-hidden">
          <div className="h-full bg-gradient-to-r from-indigo-500 to-amber-500 w-full animate-pulse" />
        </div>
  ...
  useEffect(() => {
    if (department) {
      reset({
        ...
        organization_id: department.organization_id ? String(department.organization_id) : "",
      });
    }
  }, [department, reset]);
  ...
  const onSubmit = async (data: DeptFormValues) => {
    ...
    try {
      await api.put(`/departments/${id}`, {
        ...
        // Add other fields if supported by backend update endpoint
      });
      toast.success("Department Updated!", {
        description: `${data.name} changes have been saved.`,
        icon: <CheckCircle2 className="w-4 h-4 text-emerald-500" />,
      });
      navigate("/admin/academic");
    } catch (error: any) {
      toast.error("Failed to Update Department", { description: error?.response?.data?.detail || "Could not save department changes." });
    } finally {
  ...
  if (isDeptLoading) {
    return (
      <div className="h-[70vh] w-full flex flex-col items-center justify-center gap-6">
        ...
        <p className="text-xs font-black uppercase tracking-[0.4em] text-muted-foreground animate-pulse">Synchronizing Department Data...</p>
      </div>
    );
  }
        ...
        <div className="grid lg:grid-cols-12 gap-12 items-start">
          ...
          <div className="lg:col-span-12 xl:col-span-8 flex flex-col gap-8">
            <div className="flex flex-col gap-3">
              <div className="flex items-center gap-3">
                 ...
                 <Badge variant="outline" className="border-amber-500/30 text-amber-600 dark:text-amber-400 bg-amber-500/5 px-4 rounded-full font-black uppercase tracking-[0.2em] text-[10px]">Academic Update</Badge>
              </div>
              ...
              <p className="text-lg font-medium text-muted-foreground/80 max-w-xl leading-relaxed">
                Update organizational parameters for {department?.name}.
              </p>
            </div>
                  ...
                  <div className="grid gap-x-8 gap-y-10 md:grid-cols-2">
                    ...
                    {organizations.length > 0 && (
                      <div className="space-y-3 col-span-2">
                        ...
                        <Select onValueChange={(val: string) => setValue("organization_id", val)} value={formData.organization_id}>
                    ...
                    <div className="space-y-3 group">
                      <Label htmlFor="head" className="text-[11px] font-black uppercase tracking-[0.3em] opacity-40">Head of Department</Label>
                      <div className="relative">
          ...
          <div className="hidden xl:flex xl:col-span-4 flex-col gap-8 justify-center min-h-[600px] animate-in slide-in-from-right-12 duration-1000">
             <div className="space-y-2 text-right">
                <h2 className="text-2xl font-black italic tracking-tighter text-amber-500">Preview Hub</h2>
                <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/60">Real-time state verification</p>
             </div>
                ...
                <div className="mt-8 flex flex-col gap-4 max-w-sm">
                   <div className="flex items-center gap-3 p-4 bg-white/5 dark:bg-black/40 border border-white/10 rounded-2xl backdrop-blur-sm">
                      ...
                      <p className="text-[10px] font-bold text-muted-foreground leading-tight">
                         Organizational updates trigger systemic re-indexing across all linked modules.
                      </p>
                   </div>
                </div>
             </div>
