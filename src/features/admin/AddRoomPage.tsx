...
import { 
  ...
  Wifi, Monitor, Tv, Wind, Mic2, Power
} from "lucide-react";
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-10">
                         ...
                         <Select onValueChange={(val) => setValue("type", val)} defaultValue="Lecture Hall">
                            ...
                            <SelectContent className="bg-white/95 dark:bg-black/95 backdrop-blur-2xl border-white/10 rounded-2xl shadow-2xl p-1">
                               {["Lecture Hall", "Computer Lab", "Seminar Room", "Workshop", "Research Studio"].map(t => (
                                 <SelectItem key={t} value={t} className="h-12 rounded-xl focus:bg-emerald-500/10 font-bold">
                                   {t}
                                 </SelectItem>
                               ))}
                            </SelectContent>
                  ...
                  <div className="pt-6 flex flex-col md:flex-row gap-4">
                    <Button 
                      ...
                      className="flex-1 h-16 bg-gradient-to-r from-emerald-600 to-emerald-700 hover:from-emerald-500 hover:to-emerald-600 text-white rounded-[1.25rem] text-sm font-black uppercase tracking-[0.2em] shadow-2xl shadow-emerald-600/20 active:scale-95 transition-all overflow-hidden relative group"
                    >
                ...
                <div className="mt-8 flex flex-col gap-4 max-w-sm">
                   <div className="flex items-center gap-3 p-4 bg-white/5 dark:bg-black/40 border border-white/10 rounded-2xl backdrop-blur-sm">
                      ...
                      <p className="text-[10px] font-bold text-muted-foreground leading-tight">
                         Resource scheduling prioritizes high-capacity nodes for core lecture modules.
                      </p>
                   </div>
                </div>
             </div>
