...
import { 
  ...
  History, PlaneTakeoff, GraduationCap, Flag, Loader2
} from "lucide-react";
...
const EditTermPage = () => {
  ...
  const { data: term, isLoading: isTermLoading } = useQuery({
    queryKey: ["term", id],
    queryFn: async () => (await api.get(`/terms/${id}`)).data,
  });
  ...
  useEffect(() => {
    if (term) {
      reset({
        name: term.name,
        year: term.year || "2025/2026",
        startDate: term.start_date ? term.start_date.split('T')[0] : "",
        endDate: term.end_date ? term.end_date.split('T')[0] : "",
        status: (term.status as any) || "Upcoming",
        organization_id: term.organization_id ? String(term.organization_id) : "",
      });
    }
  }, [term, reset]);
  ...
  const onSubmit = async (data: TermFormValues) => {
    ...
    try {
      ...
      toast.success("Term Updated!", {
        description: `${data.name} has been updated in the academic calendar.`,
        icon: <CheckCircle2 className="w-4 h-4 text-emerald-500" />,
      });
      navigate("/admin/academic");
    } catch (error: any) {
      toast.error("Failed to Update Term", { description: error?.response?.data?.detail || "Could not save term changes." });
    } finally {
  ...
  if (isTermLoading) {
    return (
      <div className="h-[70vh] w-full flex flex-col items-center justify-center gap-6">
        ...
        <p className="text-xs font-black uppercase tracking-[0.4em] text-muted-foreground animate-pulse">Synchronizing Registry Data...</p>
      </div>
    );
  }
        ...
        <div className="grid lg:grid-cols-12 gap-12 items-start">
          ...
          <div className="lg:col-span-12 xl:col-span-8 flex flex-col gap-8">
            <div className="flex flex-col gap-3">
              <div className="flex items-center gap-3">
                 ...
                 <Badge variant="outline" className="border-indigo-500/30 text-indigo-600 dark:text-indigo-400 bg-indigo-500/5 px-4 rounded-full font-black uppercase tracking-[0.2em] text-[10px]">Temporal Alignment</Badge>
              </div>
              ...
              <p className="text-lg font-medium text-muted-foreground/80 max-w-xl leading-relaxed">
                Update the temporal parameters for {term?.name}.
              </p>
            </div>
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-10">
                  <div className="grid gap-x-8 gap-y-10 md:grid-cols-2">
                    ...
                    <div className="space-y-3">
                      ...
                      <Select onValueChange={(val: any) => setValue("status", val)} value={formData.status}>
          ...
          <div className="hidden xl:flex xl:col-span-4 flex-col gap-8 justify-center min-h-[600px] animate-in slide-in-from-right-12 duration-1000">
             <div className="space-y-2 text-right">
                ...
                <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/60">Live feed of registry state</p>
             </div>
                ...
                <div className="mt-8 flex flex-col gap-4 max-w-sm">
                   <div className="flex items-center gap-3 p-4 bg-white/5 dark:bg-black/40 border border-white/10 rounded-2xl backdrop-blur-sm">
                      ...
                      <p className="text-[10px] font-bold text-muted-foreground leading-tight">
                         Temporal recalibration affects all scheduled events and attendance tracking metrics.
                      </p>
                   </div>
                </div>
             </div>
