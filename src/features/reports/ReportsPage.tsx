      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-10 mb-16 pt-8">
        <div className="space-y-4">
          ...
          <p className="text-slate-500 text-xl font-medium max-w-xl">
            Quickly search and analyze attendance records across the institution.
          </p>
        </div>
        ...
        <div className="flex gap-4">
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-64 rounded-[2rem] border-primary/10 backdrop-blur-2xl bg-white/90 p-3 shadow-[0_32px_64px_-12px_rgba(0,0,0,0.15)]">
              <DropdownMenuLabel className="px-4 py-3 text-[10px] font-black uppercase text-slate-400 tracking-[0.3em]">Export Format</DropdownMenuLabel>
              {[
                { label: 'CSV Spreadsheet', type: 'CSV', icon: FileSpreadsheet, color: 'text-emerald-500', bg: 'bg-emerald-500/10' },
                { label: 'Excel File', type: 'XLSX', icon: FileSpreadsheet, color: 'text-blue-500', bg: 'bg-blue-500/10' },
                { label: 'PDF Document', type: 'PDF', icon: FileText, color: 'text-rose-500', bg: 'bg-rose-500/10' },
                { label: 'JSON Data', type: 'JSON', icon: FileJson, color: 'text-amber-500', bg: 'bg-amber-500/10' },
              ].map((opt) => (
            ...
            <div className="flex items-center gap-4">
              ...
              <Select value={selectedCourse} onValueChange={setSelectedCourse}>
                ...
                <SelectContent className="rounded-3xl border-primary/10 backdrop-blur-2xl bg-white/90 p-2">
                  <SelectItem value="all" className="rounded-2xl font-black uppercase text-[10px] tracking-widest py-3 cursor-pointer">All Courses</SelectItem>
                  {courses.map(course => (
          ...
          <Table>
            <TableHeader className="bg-slate-50/50">
              <TableRow className="hover:bg-transparent border-none">
                <TableHead className="px-10 py-8 font-black uppercase text-[9px] tracking-[0.3em] text-slate-400">Course</TableHead>
                <TableHead className="px-10 py-8 font-black uppercase text-[9px] tracking-[0.3em] text-slate-400">Date & Time</TableHead>
                <TableHead className="px-10 py-8 font-black uppercase text-[9px] tracking-[0.3em] text-slate-400 hidden md:table-cell">Room</TableHead>
                <TableHead className="px-10 py-8 font-black uppercase text-[9px] tracking-[0.3em] text-slate-400">Attendance Rate</TableHead>
                ...
                <TableRow key={report.id} className="group hover:bg-white/60 transition-all duration-500 border-b border-slate-50">
                  ...
                  <TableCell className="px-10 py-8 text-slate-500 font-bold whitespace-nowrap">
                    <div className="flex flex-col">
                       ...
                       <span className="text-[10px] text-slate-400 mt-1 pl-6">TIME: {new Date(report.date).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                    </div>
                  </TableCell>
                  <TableCell className="px-10 py-8 hidden md:table-cell">
                    <div className="flex items-center gap-3">
                       ...
                       <span className="font-black text-[11px] text-slate-500 uppercase tracking-widest">{report.classroom ?? 'Off-campus'}</span>
                    </div>
                  </TableCell>
                  <TableCell className="px-10 py-8">
                    <div className="flex items-center gap-8">
                      <div className="flex flex-col">
                         ...
                         <span className="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] mt-2 italic shadow-primary">{report.present} / {report.total} STUDENTS</span>
                      </div>
                  ...
                  <TableCell colSpan={5} className="h-96 text-center border-none hover:bg-transparent">
                    <div className="flex flex-col items-center justify-center gap-6 opacity-20 group">
                       ...
                       <div className="space-y-2">
                          <span className="font-black text-2xl uppercase tracking-tighter text-slate-900">No reports found</span>
                          <p className="font-bold text-xs uppercase tracking-[0.3em] text-slate-400">We couldn't find any reports matching your search criteria.</p>
                       </div>
                    </div>
                  </TableCell>
                </TableRow>
